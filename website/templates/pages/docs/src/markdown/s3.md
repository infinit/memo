...
```
$> infinit-storage --create --as alice --s3 --name s3-silo --account s3-user --region eu-central-1 --bucket infinit-s3-storage-de-example-com --path blocks-folder
Create storage "s3-silo".
```
