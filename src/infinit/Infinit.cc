...
namespace infinit
{
  ...
  Infinit::key_value_store_get(std::string const& name)
  {
    bfs::ifstream f;
    this->_open_read(
      f, this->_key_value_store_path(name), name, "key-value store");
  ...
  Infinit::key_value_store_save(KeyValueStore const& kvs, bool overwrite)
  {
    bfs::ofstream f;
    bool existed = this->_open_write(
      f, this->_key_value_store_path(kvs.name), kvs.name, "key-value store",
      overwrite);
    save(f, kvs);
    this->report_local_action()(
      existed ? "updated" : "saved", "key-value store", kvs.name);
  }
  ...
  {
    return this->_delete(
      this->_key_value_store_path(kvs.name), "key-value store", kvs.name);
  }
  ...
  Infinit::key_value_stores_get() const
  {
    ...
    for (auto const& p
      ...
      if (is_visible_file(p))
      {
        bfs::ifstream f;
        this->_open_read(
          f, p.path(), p.path().filename().string(), "key-value store");
  ...
  {
    auto root = xdg_data_home() / "key-value-stores";
