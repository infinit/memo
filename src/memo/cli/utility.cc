...
namespace memo
{
  namespace cli
  {
      ...
      , _self(self)
    {
      ...
      this->_memo.hub_push(this->_url, "endpoints for",
                              network.name, endpoints, self, false);
    }

    InterfacePublisher::~InterfacePublisher()
    {
      this->_memo.hub_delete(
    ...
    {
      bfs::ofstream f;
      Memo::_open_write(f, path, "", "port file", true);
      f << port << std::endl;
    }
    ...
    {
      bfs::ofstream f;
      Memo::_open_write(f, path, "", "endpoint file", true);
    ...
    {
      auto res = std::vector<memo::model::Endpoints>{};
      for (auto const& peer: peers)
      {
        ...
        for (auto const& ep: eps)
          res.emplace_back(memo::model::Endpoints{ep});
      }
      return res;
    }
    ...
    namespace
    {
      ...
      {
        auto const& deps = memo::serialization_tag::dependencies;
    ...
    ensure_version_is_supported(elle::Version const& version)
    {
      if (!is_version_supported(version))
      {
        auto const supported_versions =
          elle::make_set(memo::serialization_tag::dependencies,
          [] (auto const& kv)
          {
