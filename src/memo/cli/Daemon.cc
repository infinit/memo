  ...
  namespace cli
  {
    ...
    namespace
    {
      ...
      public:
        ...
        root_path()
        {
          auto res = bfs::path("/run/user/0/memo/filesystem/daemon.pid");
          ...
          [&]
          {
            ...
            std::unique_ptr<elle::reactor::network::UnixDomainSocket> sock = [] {
              try
              {
                ...
                {
                  return std::make_unique<elle::reactor::network::UnixDomainSocket>(
                    bfs::path("/tmp/memo-root/daemon.sock"));
                }
              }
              catch (elle::reactor::network::ConnectionRefused const&)
              {
                elle::err("ensure that an instance of memo daemon is running");
              }
            }();
            std::string cmd = s + "\n";
...
//               auto res = elle::sprintf("%s/memo/filesystem/mnt", user_run_dir);
...
//             auto c = LoginCredentials{name, Memo::hub_password_hash(pass)};
//             auto json = memo.hub_login(name, c);
...
//           managers[getuid()].reset(new MountManager(memo,
//                                                     user_mount_root,
...
//                   auto lock = system_user.enter(mutex);
//                   auto res = new MountManager(memo,
//                                               peer_mount_root,
//                                               docker_mount_substitute);
...
//       }
    ...
    namespace
    {
                        ...
                        elle::json::Object const& options = elle::json::Object{})
      {
        ...
        auto users = memo.users_get();
        auto passport = boost::optional<memo::Passport>{};
        auto user = boost::optional<memo::User>{};
        ...
        if (!passport)
        {
          ...
          for (auto const& u: users)
          {
            try
            {
              passport.emplace(memo.passport_get(name, u.name));
              user.emplace(u);
              break;
            }
        ...
        auto silo_config = [&] () -> std::unique_ptr<memo::silo::SiloConfig> {
        ...
        auto network = memo::Network(
          desc.name,
          std::make_unique<memo::model::doughnut::Configuration>(
          ...
          boost::none);
        memo.network_save(*user, network, true);
        memo.network_save(std::move(network), true);
      }

      void
      acquire_network(memo::Memo& memo,
                      std::string const& name)
      {
        auto desc
          = memo.hub_fetch<memo::NetworkDescriptor>("network", name);
        memo.network_save(desc);
        try
        {
          auto nname = split(memo, name);
          auto net = memo.network_get(nname.first, nname.second, true);
        }
        catch (elle::Error const&)
        {
          link_network(memo, name);
        }
      }
    }
