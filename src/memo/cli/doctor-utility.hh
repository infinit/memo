...
namespace
{
    ...
    : public BasicResult
  {
      ...
      : public BasicResult
    {
      ...
      {
        static const auto allowed = std::regex(memo::User::name_regex());
        if (std::regex_match(name, allowed))
          ...
          return std::make_tuple(
            ...
            Result::Reason{
              elle::sprintf(
                "default system user name \"%s\" is not compatible with Memo "
  ...
  SystemSanityResults::EnvironResult::EnvironResult(
    ...
    : Result("Environment",
             ...
             (environ.empty()
              ? Result::Reason{}
              : Result::Reason{
               "your environment contains variables that will modify Memo "
  ...
  ConnectivityResults::BeyondResult::BeyondResult(Result::Reason const& r)
    : BasicResult(
      elle::sprintf("Connection to %s", memo::beyond()), !r, r)
  {}
  ...
  /// The Memo related part of the environment.
  Environ
  related_environ()
  {
  ...
  void
  _connectivity(memo::cli::Memo& cli,
                ...
                ConnectivityResults& results)
  {
    ELLE_TRACE("contact beyond")
    {
      ...
      {
        results.beyond = {
          elle::sprintf("Couldn't connect to %s", memo::beyond())
        };
      }
  ...
  void
  _system_sanity(memo::cli::Memo& cli,
  ...
  {
    bfs::ifstream i;
    memo._open_read(i, path, type, path.filename().string());
    memo::Memo::load<ExpectedType>(i);
  }

  void
  _configuration_integrity(memo::cli::Memo& cli,
                           bool ignore_non_linked,
                           ConfigurationIntegrityResults& results)
  {
    ...
    auto aws_credentials = memo.credentials_aws();
    auto gcs_credentials = memo.credentials_gcs();
    auto silos = parse(memo.silos_get());

    auto owner = [&]() -> boost::optional<memo::User>
    {
      try
      {
      ...
      catch (...)
      {
        results.user = {
          username,
          false,
          elle::sprintf("user \"%s\" is not a local Memo user", username)
        };
        return {};
      }
    }();
    ...
    auto networks = parse(memo.networks_get(owner));
...
#undef COMPARE
         ...
         ++it)
    {
      ...
      if (is_visible_file(p))
      {
        ...
        {
          // Share.
          if (is_parent_of(memo._network_descriptors_path(), p.path()))
            load<memo::NetworkDescriptor>(memo, p.path(), "network_descriptor");
          else if (is_parent_of(memo._networks_path(), p.path()))
            load<memo::Network>(memo, p.path(), "network");
          else if (is_parent_of(memo._passports_path(), p.path()))
            load<memo::Passport>(memo, p.path(), "passport");
          else if (is_parent_of(memo._users_path(), p.path()))
            load<memo::User>(memo, p.path(), "users");
          else if (is_parent_of(memo._silos_path(), p.path()))
            load<std::unique_ptr<memo::silo::SiloConfig>>(memo, p.path(), "silo");
          else if (is_parent_of(deprecated::storages_path(), p.path()))
            load<std::unique_ptr<memo::silo::SiloConfig>>(memo, p.path(), "storage");
          else if (is_parent_of(memo._credentials_path(), p.path()))
            load<std::unique_ptr<memo::Credentials>>(memo, p.path(), "credentials");
          else
  ...
  void
  _report_error(memo::cli::Memo& cli,
  ...
  void
  _output(memo::cli::Memo& cli,
          Output& out,
          Report const& results)
  {
    if (cli.script())
      memo::Memo::save(out.out, results, false);
    else
      results.print(out);
  }
}
