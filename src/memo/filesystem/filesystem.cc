...
ELLE_LOG_COMPONENT("memo.filesystem");
  ...
  namespace filesystem
  {
    ...
    FileSystem::root_address()
    {
      ...
      while (true)
      {
        try
        {
          ...
          if (auto* dht = dynamic_cast<memo::model::doughnut::Doughnut*>
            (this->_block_store.get()))
          {
    ...
    FileSystem::path(std::string const& path)
    {
      ...
      case EntryType::file:
        {
          static auto bench =
            elle::Bench<int>{"bench.filesystem.filecache.hit", 1000s};
    ...
    FileSystem::get(bfs::path path, model::Address address)
    {
      ...
      static auto bench =
         elle::Bench<int>{"bench.filesystem.dircache.hit", 1000s};
      boost::optional<int> version;
