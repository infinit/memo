...
namespace kademlia
{
  ...
  public:
    Kademlia(Configuration const& config,
             std::shared_ptr<memo::model::doughnut::Local> server,
             memo::model::doughnut::Doughnut* doughnut);
    virtual ~Kademlia();
    void store(memo::model::blocks::Block const& block);
    void remove(Address address);
    void fetch(Address address, std::unique_ptr<memo::model::blocks::Block> & b);
  ...
  protected:
    void
    _discover(memo::overlay::NodeLocations const& peers) override;
    MemberGenerator
    _allocate(memo::model::Address address,
            int n) const override;
    MemberGenerator
    _lookup(memo::model::Address address,
            int n,
            bool fast) const override;
    Overlay::WeakMember
    _lookup_node(memo::model::Address address) const override;
  ...
  private:
    ...
    void _ping();
    void _refresh();
    void _reload(memo::model::doughnut::Local& l);
    void _bootstrap();
    void _cleanup();
    void _republish();
...
namespace memo
{
  namespace overlay
  {
    namespace kademlia
    {
      ...
      {
        using Self = memo::overlay::kademlia::Configuration;
        ...
        make(std::shared_ptr<memo::model::doughnut::Local> local,
