...
#define REGISTER(classname, type)                               \
        ...
        {
          RequestKey(memo::model::doughnut::Passport p)
            : passport(p)
          {}
          ...
          memo::model::doughnut::Passport passport;
          elle::Buffer token;
        ...
        {
          KeyReply(memo::model::doughnut::Passport p)
            : passport(p)
          {}
...
#undef REGISTER
        ...
        , _terminating(false)
      {
        ...
        if (auto l = local)
        {
          l->on_fetch().connect(
            [this]
            (Address a, std::unique_ptr<memo::model::blocks::Block> & b)
            {
              this->fetch(a, b);
            });
          l->on_store().connect(
            [this](memo::model::blocks::Block const& b)
            {
              this->store(b);
            });
          l->on_remove().connect(
      ...
      Node::gossipEmitter()
      {
        ...
        elle::reactor::sleep(std::chrono::milliseconds(v));
        packet::Gossip p;
        p.sender = _self;
        p.observer = _observer;
        while (true)
        {
          elle::reactor::sleep(std::chrono::milliseconds(_config.gossip.interval_ms));
          p.contacts.clear();
          p.files.clear();
                       ...
                       std::function<void (std::pair<Address, NodeLocation>)> yield)
      {
        ...
        for (int i = 0; i < _config.query_get_retries; ++i)
        {
          ...
          elle::reactor::wait(r->barrier,
                              std::chrono::milliseconds(_config.query_timeout_ms));
          if (!r->barrier.opened())
          {
        ...
        auto f = [this,file,n,local_override, attempts, yield, query_node, fast_mode, ignore_local_cache]() {
          ...
          if (!query_node && fg == _group && !ignore_local_cache)
          {
            ...
            auto it_us = boost::find_if(its,
              [&](std::pair<const memo::model::Address, File> const& f) {
      ...
      */
      ...
      void
      Node::store(memo::model::blocks::Block const& block)
      {
      ...
      auto
      Node::_lookup(std::vector<memo::model::Address> const& addresses,
                    int n) const
        -> LocationGenerator
      {
      ...
      auto
      Node::_allocate(memo::model::Address address, int n) const
        -> MemberGenerator
      {
        BENCH("allocate");
      ...
      auto
      Node::_lookup(memo::model::Address address,
