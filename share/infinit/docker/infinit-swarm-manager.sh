...
# Check if network infra is already there
if kv $KV_URL get infinit/users/cluster_node; then
  ...
  kv $KV_URL get infinit/networks/cluster_network.storage > $data_dir/networks/cluster_manager/cluster_network
  ...
  kv $KV_URL set infinit/users/cluster_manager "$(infinit-user --export --full cluster_manager)"
  ...
  kv $KV_URL set infinit/networks/cluster_network.storage \
      "$(cat ~/.local/share/infinit/filesystem/networks/cluster_manager/cluster_network)"
