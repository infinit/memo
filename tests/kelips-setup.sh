...
for i in $(seq 1 $nodes); do
  ...
  INFINIT_HOME=$rootdir/conf$i infinit-network --link --as $user$i --name $user/kelips --storage storage $(port $i $port_base $nports)
done
...
for i in $(seq 0 $observers); do
  ...
  INFINIT_HOME=$rootdir/observer_conf_$i infinit-network --link --as obs$i --name $user/kelips
done
...
echo "INFINIT_HOME=$rootdir/conf0 infinit-volume --run --as $user --name $user/kelips --mountpoint mount_0 --async --cache --allow-root-creation &" >> $rootdir/run-nodes.sh
echo 'sleep 2' >> $rootdir/run-nodes.sh
for i in $(seq 1 $nodes); do
  echo "INFINIT_HOME=$rootdir/conf$i infinit-network --run --as $user$i --name $user/kelips --peer 127.0.0.1:$port_base &" >> $rootdir/run-nodes.sh
done
