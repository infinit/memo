...
mkdir store0
mkdir conf0
INFINIT_DATA_HOME=$rootdir/conf0 infinit-user --create --name $user
exported_user=$(INFINIT_DATA_HOME=$rootdir/conf0 infinit-user --export --name $user)
for i in $(seq 1 $nodes); do
  mkdir store$i
  mkdir conf$i
  INFINIT_DATA_HOME=$rootdir/conf$i infinit-user --create --name $user$i
  echo $exported_user | INFINIT_DATA_HOME=$rootdir/conf$i infinit-user --import
  INFINIT_DATA_HOME=$rootdir/conf$i infinit-user --export --name $user$i | INFINIT_DATA_HOME=$rootdir/conf0 infinit-user --import
done

# Generate storages
for i in $(seq 0 $nodes); do
  INFINIT_DATA_HOME=$rootdir/conf$i infinit-storage --create --capacity 2000000000 --name storage --filesystem --path $rootdir/store$i
done
...
for i in $(seq 0 $observers); do
  ...
  INFINIT_DATA_HOME=$rootdir/observer_conf_$i infinit-user --create --name obs$i
  echo $exported_user | INFINIT_DATA_HOME=$rootdir/observer_conf_$i infinit-user --import
  INFINIT_DATA_HOME=$rootdir/observer_conf_$i infinit-user --export --name obs$i | INFINIT_DATA_HOME=$rootdir/conf0 infinit-user --import
  echo $exported_network | INFINIT_DATA_HOME=$rootdir/observer_conf_$i infinit-network --import
...
# create volume
INFINIT_DATA_HOME=$rootdir/conf0 infinit-volume --create --name kelips \
  --as $user --network kelips


exported_volume=$(INFINIT_DATA_HOME=$rootdir/conf0 infinit-volume --export --as $user --name kelips)
for i in $(seq 0 $observers); do
  echo $exported_volume | INFINIT_DATA_HOME=$rootdir/observer_conf_$i infinit-volume --import --mountpoint observer_mount_$i
done
...
for i in $(seq 0 $observers); do
  echo "INFINIT_DATA_HOME=$rootdir/observer_conf_$i infinit-volume --run --mountpoint observer_mount_$i --as obs$i --name $user/kelips --peer 127.0.0.1:$port_base --cache --async" > $rootdir/run-volume-$i.sh
